server <- function(input, output) {
  ...
  output$atentados_por_ano <- renderPlot({
    ...
    ## Gera o gráfico...
    gg <- ggplot(data = atentados_por_ano_filtro, aes(x = ano, y = atentados)) +
      # Gera linha no gráfico
      geom_line() +
      
      # Gera pontos em cada ano
      geom_point(data = atentados_por_ano_filtro, aes(x = ano, y = atentados), 
                 pch = 21, fill = 'white', color = 'black', alpha = 1, size = 3) +
      
      # Define labels
      labs(x = 'Ano', y = 'Número de atentados', caption = 'Fonte: Global Terrorism - https://www.kaggle.com/START-UMD/gtd') +
      
      # Define escala de X
      scale_x_continuous(limits = c(ano_min, ano_max), breaks = seq(ano_min, ano_max, 3)) +
  ...
  output$grupos_mais_atuantes <- renderPlot({
    ...
    #atentados_por_grupo <- atentados_por_grupo[order(atentados_por_grupo$atentados, decreasing = TRUE),]
    ...
    ## Gera o gráfico
    gg <- ggplot(data = atentados_por_grupo, aes(x = organizacao_terrorista, y = atentados)) +
      geom_bar(stat = 'identity', aes(fill = atentados), width = 0.8) +
      
      # Define labels
      labs(x = '', y = 'Número de atentados', caption = 'Fonte: Global Terrorism - https://www.kaggle.com/START-UMD/gtd') +
  ...
  output$atividades_grupos_mais_atuantes <- renderPlot({
    ...
    ## Gera o gráfico
    p01 <- ggplot(data = atividade_por_grupo, aes(x = ano, y = atentados, col = organizacao_terrorista)) +
      geom_line() +
      
      # Define labels
      labs(x = '', y = '', color = 'Organização terrorista', caption = 'Fonte: Global Terrorism - https://www.kaggle.com/START-UMD/gtd') +
